...
public class RedissonLock extends RedissonExpirable implements RLock {
    ...
    public void lockInterruptibly() throws InterruptedException {
        //leaseTime被称作是租约时间，这里是-1所以是默认永久性的持有锁
...
//        get(lockAsync(leaseTime, unit));
    ...
    private <T> RFuture<Long> tryAcquireAsync(long leaseTime, TimeUnit unit, final long threadId) {
        ...
        //leaseTime是-1则按照看门狗的超时时间30s，进行加锁
    ...
    <T> RFuture<T> tryLockInnerAsync(long leaseTime, TimeUnit unit, long threadId, RedisStrictCommand<T> command) {
        ...
        //redis获取锁的核心逻辑
        //1. 首先通过exists key判断是否存在该锁，如果返回为0证明锁不存在则使用`hset lockName id:threadId 1`进行加锁，同时设置过期时间为30秒(pexpire lockName 30000)
        //2. 如果当前的锁存在且是自己加的锁，就会对当前锁的计数进行+1操作，同时将锁的过期时间重置为30s
        //3. 锁已经存在且锁不是自己持有的那么，就会返回该锁的剩余过期时间。
